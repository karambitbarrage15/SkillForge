...
func NewPool(count int, groupName string, consumer queue.Consumer, acker queue.Acker, hb Heartbeater, proc Processor, interval, ttl time.Duration) *Pool {
	if count <= 0 {
	...
	for i := 0; i < count; i++ {
		...
		workers[i] = NewWorker(workerID, groupName, consumer, acker, hb, proc, interval, ttl)
	}
